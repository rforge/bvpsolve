...
out   <- mebdfi(y,times,Pollution,parms=NULL)

# increasing tolerance
out2  <- mebdfi(y,times,Pollution,parms=NULL,
...
TT    <- c(0,60)
s0<-system.time(
Test0 <- mebdfi(y,TT,Pollution,parms=NULL,atol=1e-16,rtol=1e-16,verbose=TRUE)
)["elapsed"]
...
print( cbind(mebdfi =(Test0[2,2:21]-ytrue),
             vode =(Test1[2,2:21]-ytrue),
             lsode =(Test2[2,2:21]-ytrue),     
             lsoda =(Test3[2,2:21]-ytrue),
             lsodes=(Test4[2,2:21]-ytrue),
             daspk =(Test5[2,2:21]-ytrue))    )

DF <- data.frame(method=c("mebdfi","vode","lsode","lsoda","lsodes","daspk"),
"maximal deviation"=c(max(abs(Test0[2,2:21]-ytrue)),max(abs(Test1[2,2:21]-ytrue)),
max(abs(Test2[2,2:21]-ytrue)),max(abs(Test3[2,2:21]-ytrue)),
max(abs(Test4[2,2:21]-ytrue)),max(abs(Test5[2,2:21]-ytrue))),
"timing"=c(s0,s1,s2,s3,s4,s5))
