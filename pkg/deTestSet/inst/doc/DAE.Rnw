...
\title{Solving Differential Algebraic
  Equations in \proglang{R}}

\Plaintitle{Solving Differential Algebraic
  Equations in R}
...
\Abstract{
  Function \code{mebdfi} from \R package \dae solves initial value
...
\begin{itemize}
  \item   \ds (Soetaert, Petzoldt, Setzer, 2009) for initial value problems 
  of ordinary and partial
  differential equations, and simple differential algebraic equations,
  \item \bvp (Soetaert, Cash, Mazzia, 2009) for boundary value problems, and
  \item \rs (Soetaert 2009), to solve steady-state conditions
\end{itemize}

Package \code{deTestSet} implements a solver for differential algebraic
...
For details about this method, see Cash (1979, 1983), Cash and Considine (1992).

In this package vignette, the new solver is tested against the DAE test
problems from the following site:
 \url{http://www.dm.uniba.it/~testset}  (Mazzia and Magherini, 2008).
...
dyini <- with(as.list(parameter),c(51.338775,51.338775,-yini[2]/(c2*r3),
    -24.9757667,-24.9757667,-83.333333333,-10.00564453,-10.00564453))
@
...
in the package the DLL name is equal to the package name, deTestSet}
...
@
The stifness and damping options are set with ipar:
<<>>=
nind   <- c(14,10,0)
...
First the index of the variables, the initial conditions and the 
...
Karline Soetaert, Francesca Mazzia, Jeff Cash (2010). deTestSet: 
  ...
  Differential Algebraic Equations (DAE). R package version 0.0.
  
 Karline Soetaert, Thomas Petzoldt and R. Woodrow Setzer (2010). deSolve: General
  solvers for initial value problems of ordinary differential equations (ODE),
  partial differential equations (PDE), differential algebraic equations (DAE)
  and delay differential equations.
  R package version 1.7.

Karline Soetaert, Jeff Cash and Francesca Mazzia (2009).
  bvpSolve: Solvers for boundary value problems of ordinary
  differential equations. R package version 1.1.  
  
Karline Soetaert (2009).  rootSolve: Nonlinear root finding,
  equilibrium and steady-state analysis of ordinary
