      SUBROUTINE DOPRI6(N,FCN,X,Y,XEND,
     &                  RTOL,ATOL,ITOL,
     &                  SOLOUT,IOUT,
...
C     VSTI IS AN ARRAY OF WORKSPACE OF LENGTH N. IT IS NOT NEEDED
C          IS AUTOMATIC STIFFNESS DETECTION IS NOT USED.
C
C-----------------------------------------------------------------------
C 
C     SOPHISTICATED SETTING OF PARAMETERS
C     -----------------------------------
C           SEVERAL PARAMETERS (WORK(1),...,IWORK(1),...) ALLOWS US
...
C              CAUSES A VALUE BETA=0 TO BE SET.
C              CAUSES 0.04D0.
C
...
C              IF IWORK(4)=1 THEN WE TEST AT EACH STEP.
C
...
C
C     IDID        REPORTS ON SUCCESSUPON RETURN:
