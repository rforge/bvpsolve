...
C -------- NRDENS   NUMBER OF DENSE OUTPUT COMPONENTS
      ...
      ELSE
         IF(NRDENS.GT.0.AND.IOUT.LT.2)THEN
            IF (IPRINT.GT.0) THEN
         CALL Rprint(' Warning: put IOUT=2 or IOUT=3 for dense output')
            ENDIF
         END IF 
         IF (NRDENS.EQ.N) THEN
            DO I=1,NRDENS
...
C --- INITIAL PREPARATIONS   
      ...
      HMAX=ABS(HMAX)     
      IORD=8  
      IF (H.EQ.0.D0) H=HINIT(N,FCN,X,Y,POSNEG,K1,K2,K3,IORD,
...
C --- FAIL EXIT
  ...
  77  CONTINUE
      IF (IPRINT.GT.0) THEN
         CALL Rprintd1('Exit of DOP853 at x = ', X) 
      ENDIF
  ...
  79  CONTINUE
      ...
      END
C
      FUNCTION HINIT(N,FCN,X,Y,POSNEG,F0,F1,Y1,IORD,
     &                       HMAX,ATOL,RTOL,ITOL,RPAR,IPAR)
C ----------------------------------------------------------
...
C ks: make this a subroutine 


      SUBROUTINE  CONTD8(NEQ,X,CON,ICOMP,ND,VAL)
C ----------------------------------------------------------
