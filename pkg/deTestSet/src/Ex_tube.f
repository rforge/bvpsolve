...
c      
      ...
      call tubefunc(n,x,y,delta,rpar,ipar)
C
      do i=1,18
...
c-----------------------------------------------------------------------
      ...
      SUBROUTINE tubefunc(NEQN,T,Y,DF,RPAR,IPAR)
      INTEGER NEQN
      ...
      do 20 j=1,nnodes         
         do 10 i=1,nnodes            
            if (lambda(i,j) .lt. 0d0) then
               ierr = -1
        CALL Rprintd1('An error occurred in TUBE, at time ', T)
         call rexit('stopped')
               return
            endif
