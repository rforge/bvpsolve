...
\Keywords{ordinary differential equations, boundary value problems,
  shooting method, collocation method, \proglang{R}}

\Plainkeywords{ordinary differential equations, boundary value problems,
  shooting method, collocation method, R}
...
\Abstract{
  ...
  that can be found on \url{http://www.ma.ic.ac.uk/~jcash/BVP_software},
...
%\VignetteIndexEntry{bvpSolve: Test Problem Set}
...
method, but it is solvable by collocation
<<>>=
xi <-0.1
twp   <- bvptwp(yini=c(1,NA),yend=c(0,NA),x=seq(0,1,by=0.01),
...
\subsection{problem 7}
  \begin{eqnarray*}
    ...
    y(1) &=& 1
  \end{eqnarray*}
This problem cannot be solved with the shooting method for large xi.
<<>>=
...
\begin{figure}
\begin{center}
<<label=prob7,fig=TRUE,echo=FALSE>>=
<<prob7>>
@

\end{center}
\caption{Solution of the BVP ODE problem 7, y versus x- see text for R-code}
\label{fig:pr7}
\end{figure}
<<label=prob7b,include=FALSE>>=
plot(twp[,1],twp[,3],type="l",main="test problem 7",
  col="blue",xlab="x",ylab="y'")
lines(mod[,1],mod[,3],col="red")
lines(twp2[,1],twp2[,3],col="green")
...
\begin{figure}
\begin{center}
<<label=prob7b,fig=TRUE,echo=FALSE>>=
<<prob7b>>
@
...
\clearpage
 ...
 \subsection{problem 17}
  \begin{eqnarray*}
   y'' =-3 \xi y/(\xi +x^2)^2\\
   y_{(x=0.1)} = -y(-0.1) = 0.1 \sqrt(\xi + 0.01)
  \end{eqnarray*}
...
@
Neither shooting nor bvptwp work...

and bvpcolmod works only with eps sufficiently large...
<<>>=
xi <-0.03
mod1  <- bvpcolmod(yini=ini,yend=end,x=seq(0,1,by=0.01),
...
Now come a series of similar problems (problem 25-29), that differ only
...
twp2  <- bvptwp(yini=ini,yend=end,x=seq(0,1,by=0.01),
        func=Prob25, guess=0,xi=xi)
@
This problem can also be solved with bvpcolmod, but only when treating $\xi$
as a continuation parameter (\code{eps}):
<<>>=
xi <- 0.001
mod  <- bvpcolmod(yini=ini,yend=end,x=seq(0,1,by=0.01),
...
But both collocation methods do...
<<>>=
xi <-0.1
twp  <- bvptwp(yini=ini,yend=end,x=seq(0,1,by=0.01),
...
lines(mod[,1],mod[,4],col="red")
lines(mod2[,1],mod2[,4],col="green")
