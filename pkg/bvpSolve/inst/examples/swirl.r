...
x     <- seq(A,B,len=100)
print(system.time(Sol2 <- bvpcolmod(atol=1e-5,x=x,func=fsub,guess= c(2,0,0),
                  yini=c(-1,NA,0,0,NA,NA), yend=c(1,NA,0,0,NA,NA),eps=eps)))
...
print(system.time(Sol3 <- bvpcolmod(atol=1e-8,x=x,func=fsub,guess= c(2,0,0),
  yini=c(-1,NA,0,0,NA,NA),
  yend=c(1,NA,0,0,NA,NA),eps=eps)))
