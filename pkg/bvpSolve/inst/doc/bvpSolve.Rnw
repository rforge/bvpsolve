...
Given here without derivation, the augmented derivative function ; variable
...
left boundary; boundary condition 2 sets the left boundary of \code{y_1} 
...
@
This also requires good initial conditions to find a solution:
<<>>=
