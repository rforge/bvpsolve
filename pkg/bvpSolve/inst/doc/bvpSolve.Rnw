...
\section{Introduction}
  ...
  function \code{f} is the differential equation, and $g_a$ and $g_b$ the
  ...
  \begin{itemize}
    ...
    \item \code{bvptwp}, the mono-implicit Runge-Kutta (MIRK)
      method with deferred corrections, based on the FORTRAN code TWPBVP
      \citep{Cash91}.
  \end{itemize}
  ...
  equations and of boundary conditions are given.  However, input
  ...
  However, in order to speed-up the simulations, it is possible to provide 
...
@
Here we produce the output with the S3 plot method, which depicts both 
dependent variables:
...
\section{a BVP problem including an unknown parameter}
  ...
  Mathieus equation (parameter $\lambda$) is computed. The equation:
  \[
...
guess of the missing initial conditions is necessary; the initial guess for the 
...
This is done by assuming the boundary conditions as "parameters", and using the 
strategy of defining extra variables, similar as in previous section.

Given here without derivation, the augmented derivative function is (variable
3 is the unknown parameter \code{T}, variables 4 and 5 the initial conditions of 
\code{y_1}, and \code{y_2} respectively):
<<>>=
