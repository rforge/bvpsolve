...
\section{Introduction}
  ...
  \bs can only solve sets of first-order ODEs.
  ...
  Two BVP solvers are included in \bs:
  \begin{itemize}
    \item \code{bvpshoot}, implementing the shooting method. This method
      ...
      packages \ds and \rs.
    \itme \code{bvptwp}, the mono-implicit Runge-Kutta (MIRK)
  ...
  Therefore, by default \bs function \code{bvptwp} numerically
  ...
  In this package vignette it is shown how to formulate and solve BVPs.
  We start with a simple example, comprising a second-order ODE (one from
  the test problems of Jeff Cash), followed
  ...
  How to implement more complex initial conditions is then examplified by
  means of problem "musn". Next, solving for the fourth eigenvalue of
  "Mathieu's equation" illustrates how to solve a BVP
  including an unknown parameter. The "elastica" problem is used
  to demonstrate how to specify the analytic jacobians, and how to implement
  problems in \proglang{FORTRAN} or \proglang{C}. Finally, a linear testcase
  which has a steep boundary layer is implemented in \proglang{FORTRAN},
  ...
  See also document bvpSolve: "a set of 35 test Problems" from the package's site
  on CRAN:
...
By default, the Jacobian of the derivative function and of the boundary
...
and the DLL created and loaded; This model has been made part of package \bs,
