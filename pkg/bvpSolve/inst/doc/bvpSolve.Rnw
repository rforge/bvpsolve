...
\Abstract{\R package \bvp \citep{bvpSolve} is designed for the numerical
  ...
  \begin{itemize}
    ...
    \item function \code{bvpcol}, the collocation method 
       based on FORTRAN code COLNEW
      \citep{Bader87}, for solving Multi-point  boundary value problems of
      mixed order
  \end{itemize}
 ...
 This is a vignette of package \rs.
}
...
\subsection{Package bvpSolve}
  ...
  Functions \code{bvpshoot} and \code{bvptwp} can only solve sets of 
  \emph{first-order} ODEs.
  ...
  \code{bvpcol} can also solve higher-order ODEs without writing them as a set
  of first-order ODEs, but this makes the 
  interface a bit more difficult.
...
First the second and fourth order equations are rewritten as a set of $1_{st}$ 
...
Function \code{bvpcol} can also solve the same problem without rewriting it
in $1^{st}$ order form.
...
(but not quite), the second way is significantly faster.
<<>>=
