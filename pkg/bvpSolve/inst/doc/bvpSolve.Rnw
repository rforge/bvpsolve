...
\Keywords{ordinary differential equations, boundary value problems,
  shooting method, collocation method, mono-implicit Runge-Kutta,
  \proglang{R}}

\Plainkeywords{ordinary differential equations, boundary value problems,
  shooting method, collocation method, mono-implicit Runge-Kutta, R}
...
\Abstract{This document is about package \bvp \citep{bvpSolve},
  ...
  solution of boundary value problems for (first-order) ordinary differential
  equations (ODE) in \R.
  ...
  \begin{itemize}
    ...
    \item function \code{bvpcolmod}, the higher-order collocation method COLMOD
      \citep{Cash95}, a variant from the well-known code COLSYS/COLNEW
      \citep{Asher81}, \citep{Bader87}.
  \end{itemize}
...
%\VignetteIndexEntry{boundaryValueProblems}
...
print(system.time(
sol3  <- bvpcolmod(yini=c(-1,NA),yend=c(1,NA),x=x,func=fun, guess=0)
))
@
  ...
  two methods. However, there are particular problems where
  \code{bvpshoot} does not lead to a solution, whereas the collocation and MIRK
  methods do (see below).

  The plot shows that the three methods give the same solution:
<<label=pr7a,include=FALSE>>=
plot(sol2[,1],sol2[,3],type="l",main="test problem 7, ksi=0.1",
     lwd=2,col="red")
points(sol3[,1],sol3[,3],col="blue")
points(sol1[,1],sol1[,3],col="green",pch="x")
legend("topright",c("bvptwp","bvpcolmod", "bvpshoot"),
        lty=c(1,NA,NA), pch=c(NA,1,3),col=c("red","blue","green") )
@
...
However, it can still easily be solved with the collocation and MIRK methods:
<<>>=
ks <-0.0001
...
print(system.time(
sol3  <- bvpcolmod(yini=c(-1,NA),yend=c(1,NA),x=seq(-1,1,by=0.01),
           func=fun, guess=0)
))
@
...
solved using \code{bvptwp} and (at least on windows platforms)
also with \code{bvpcolmod}:
<<>>=
print(system.time(
...
print(system.time(
Solmod <- try(bvpcolmod(x=x,func=fsub,guess= c(2,0,0), atol=1e-5,
                  yini=c(-1,NA,0,0,NA,NA), yend=c(1,NA,0,0,NA,NA)),
    silent=TRUE)
))
@
...
is given (only applicable for \code{bvptwp}):
<<>>=
eps <- 0.001
xguess <- x
...
\section{Automatic Continuation With Collocation}
  Function \code{bvpcolmod} implements fortran function \code{Colmod}
  \citep{Cash95} which is a revised version of the package Colnew \citep{Bader87}.
  It has been adapted to allow an automatic continuation strategy.

  Here we implement problem "shockwave" \citep{Asher95}.

  We start by declaring the global problem dependent parameters
<<>>=
eps     <- 0.01
gamma   <- 1.4
@
  The derivative function contains the continuation parameter \code{eps}:
<<>>=
...
<<label=eps,include=FALSE>>=
plot(Sol, type="l", lwd=2, ylab="f1",main="Schockwave")
@
\setkeys{Gin}{width=0.4\textwidth}
\begin{figure}
\begin{center}
<<label=eps,fig=TRUE,echo=FALSE>>=
<<eps>>
@
\end{center}
\caption{Solution of the BVP ODE problem using automatic continuation,
  see text for R-code}
\label{fig:mat}
\end{figure}
\clearpage
