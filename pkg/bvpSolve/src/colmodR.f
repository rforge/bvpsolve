...
C.... changed name consts into xconsts, to avoid conflict with twpbvp
C.... ******************************************************************
...
C.... xConsts - Is Called Once By Colmod  To Initialize Constants
...
cks: this cannot be printed - too much -> put it In R-code
c      Write (msg,1000) Ncomp, (M(Ip), Ip = 1,Ncomp)
c         CALL rwarn(msg)
c      Go To 70
   60 continue 
c   60 Write (msg,1001) Ncomp, (M(Ip), Ip = 1,Ncomp)
c         CALL rwarn(msg)

c   70 Write (msg,1002) (Zeta(Ip), Ip = 1,Mstar)
c         CALL rwarn(msg)

c      If ( Nfxpnt .gt. 0 ) THEN
c         Write (msg,1003) Nfxpnt, (Fixpnt(Ip), Ip = 1,Nfxpnt)
c         CALL rwarn(msg)
c      ENDIF
c      Write (msg,1004) K
c	   CALL rwarn(msg)
...
c ks: too much writing -> toggled off
c      If (Iflag .eq. 1 .and. Iprint .eq. 0) Then
c         Write(msg,1020) ('Z',Ltol(J), J=1,Ntol)
c         CALL rwarn(msg)
c         Jstep = Max(N/30,1)
c         Iadd = Jstep*Mstar
c         Ind = N + 1 
c         Do 390 I = 1, N, Jstep
...
c      Endif
      ...
      Return
C-----------------------------------------------------------------------
c 1000 Format(' The Number Of (Linear) Diff Eqns Is ', I3,
c     +     ' Their Orders Are', 20i3)
c 1001 Format('The Number Of (Nonlinear) Diff Eqns Is' , I3/,
c     +     'Their Orders Are', 20i3)
c 1002 Format(' Side Condition Points Zeta', 8f10.6, 4( / 27x, 8f10.6))
c 1003 Format(' There Are ',I5,'Fixed Points In The Mesh - ',
c     +     10(6f10.6/))
c 1004 Format(' Number Of Colloc Pts Per Interval Is', I3)
c 1005 Format(' Components Of Z Requiring Tolerances -',8(7x,I2,1x),
c     +     4(/38x,8i10))
c 1006 Format(' Corresponding Error Tolerances -',6x,8d10.2,
c     +     4(/39x,8d10.2))
c 1007 Format(' Initial Mesh(Es) And Z,Dmz Provided By User')
c 1008 Format( ' The Initial Value Of Epsilon Is ',D11.4,/,
c     +     ' The Desired Final Value Of Epsilon Is ',D11.4)
 1009 Format(' The Maximum Number Of Subintervals Is Min (', I4,
     +     ' (Allowed From Fspace),',I4, ' (Allowed From Ispace) )')
 1010 Format(' Insufficient Space To Double Mesh For Error Estimate')
 1011 Format (' Continuation Step ',I2,', Epsilon = ',D9.4)
 1012 Format ('This Is The Final Continuation Step Since ',
     +        'Maxcon =',I4)
 1013 Format (' Tolerances Satisfied For Final Problem',
     +        ' Epsilon = ',D9.4)
 ...
 1016 Format (' Storage Limit Being Approached, Change Epsmin To ',
     +     D9.4)
 1017 Format (' Machine Precision Possibly)Not Sufficient For Eps <',
     +     D9.4) 
...
C.... Exit If Expected N Is Too Large (But May Try N = Nmax Once)
 ...
 480  If ( Iconv .eq. 0 )  Imesh = 1
      Go To 20
C     ---------------------------------------------------------------
 1000 Format(' A Local Elimination Matrix Is Singular ')
 1001 Format(' The Global Bvp-Matrix Is Singular ')
 1002 Format(' Fixed Jacobian Iterations,')
 1003 Format(' Iteration = ', I3, ' Norm (Rhs) = ', D10.2)
 1004 Format(' Convergence After ', I3,' Iterations' )
 1005 Format(' Switch To Damped Newton Iteration,')
 1006 Format(' Full Damped Newton Iteration,')
 1007 Format(' It = ',I3,' Relaxation Factor = ',D10.2
     +       ' Norm Of Scaled Rhs Changes From ',D10.2,' To',D10.2
     +       ' Norm Of Rhs Changes From  ',D10.2,' To',D10.2,
     +       D10.2)
 1008 Format(' Relaxation Factor Corrected To ', D10.2
     +       ' Norm Of Scaled Rhs Changes From',D10.2,' To',D10.2
     +       ' Norm Of Rhs Changes From',D10.2,' To',D10.2
     +       ,D10.2)
 1009 Format(' Mesh Values For Z(', I2, '),' )
 1010 Format( 8d15.7)
 1011 Format(' No Convergence After ', I3,' Iterations')
 1012 Format(' No Convergence.  Relaxation Factor =',D10.3
     +       ,' Is Too Small (Less Than', D10.3, ')')
 1013 Format('** Mesh Cannot Be Defined Within The Bounds Imposed',
     +       ' By The Machine Precision')
 1014 Format('  (No Convergence) ')
 1015 Format(' (Probably Tolerances Too Stringent, Or Nmax Too small)')
 1016 Format(' Convergence After ', I3,' Iterations ')
      End
...
c         Endif 
      ...
      Return
C----------------------------------------------------------------
c 1000 Format(/' The Former Mesh (Of',I5,' Subintervals),',
c     +       600(/8f12.6))
 1001 Format ('  Expected N Too Large  ')
 1002 Format(' Mesh Selection Info, Degree Of Equidistribution = '
     +     , F8.5, ' Prediction For Required N = ', I8)
c 1003 Format(/' The New Mesh (Of',I5,' Subintervals),' ,600(/8f12.6))
 1004 Format(' The New Mesh Has',I5,' Subintervals')
c 1005 Format(/' The Initial Mesh (Of',I5,' Subintervals),' ,
c     +     600(/8f12.6))
 1006 Format(' The Initial Mesh Has',I5,' Subintervals')
 1007 Format(' Halving Mesh (And Then Doubling) In Order To',
     +     ' Calculate An Error Estimate')
      End

      Subroutine xConsts (K, Rho, Coef)
C
C**********************************************************************
C
C   Purpose
...
C
   ...
   70 Continue
      Return
C---------------------------------------------------------------------
 1000 Format (' The Estimated (Mixed Relative/Absolute) Errors Are,')
 1001 Format (' U(', I2, ') -',4d12.4)
      End
C---------------------------------------------------------------------
C                            P A R T  3
...
C
  ...
  170 Continue
      Return
C--------------------------------------------------------------------
 1000 Format(' ** Domain Error In Approx **',
     +       ' X = ',D20.10, '   Aleft = ',D20.10,
     +       '   Aright = ',D20.10)
      End
