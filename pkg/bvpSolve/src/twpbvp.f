...
*  in the user's guide, available as a LaTeX file from netlib.
*  Machine precision is provided automatically via netlib
*  (in the routine d1mach).
*  Questions should be directed to Professor Jeff Cash, Department
*  of Mathematics, Imperial College, London SW7 2BZ, United Kingdom,
*  email j.cash@ic.ac.uk.


c ks: changed function call to also include rpar, ipar...
c ks: pass precision from C-code...
...
*  invalid, exit with the flag iflbvp set to -1.
      ...
      if (givmsh .and. nmsh .lt. nfxpnt+2) THEN
        write(msg,*)"initial mesh should have more points than x"
        call rwarn(msg)
        return
      endif
      if (givmsh .and. xx(1) .ne. aleft) then
        write(msg,*)"xx(1) ne aleft"
        call rwarn(msg)
        return
      endif
      if (givmsh .and. xx(nmsh) .ne. aright) then
        write(msg,*)"xx(end) ne aright"
        call rwarn(msg)
        return
      endif
      if (nfxpnt .gt. 0) then
...
*  given floating-point and integer workspace.
  ...
  901 format('nmax from workspace =',i8)
...
*  Partition integer workspace.
     ...
     *   fsub,dfsub,gsub,dgsub,iflbvp,precis, rpar, ipar)
...
*     end of dssq  .
     ...
     *   precis, rpar, ipar)
      ...
      dimension  xx(*), u(nudim, *), rpar(*), ipar(*), precis(3)
...
* Error exit---too many mesh points.
  ...
  901 format('epsmch',1pe10.3)  
  902 format('initial mesh')
  903 format('start 4th order, nmsh',i5)
  904 format('do not go on to 6th')
  905 format('start 6th order')
  906 format('start 8th order')
      end
...
*     end of logic for .not. onto6
  ...
  908 format('reposs')
  909 format('savedu')
  910 format('onto6 after decid4')
  911 format('rat1,oldrt1',2(1pe11.3))
  912 format('reposs and not onto6')
  913 format('drat,u,tol',3(1pe11.3))
...
*  be adjusted.
 ...
 9994    format('***in fail6')
         write(msg,9993) remax, eight*tol(itlmx)
         call rwarn(msg)

 9993    format('remax',1pe14.4,5x,'8*tol',1pe14.4)
...
*     end of logic for err8 greater than err6
  ...
  901 format('conv8')
  902 format('err6, err8, er6old, er8old',4(1pe11.3))
  903 format('8th order fails error tests.')
      end
...
*         if (pdebug) write(6,904) im, texp, timp, ratdc(im), dfctol
  ...
  901 format('dccal')
  902 format('dfexmx, incmp, inmsh, intol',1pe11.3,3i5)
  903 format('derivm',1pe11.3)
  904 format('im, texp, timp, ratdc, dfctol',i5,4(1pe11.3))
  905 format('rat1, rat2, dfimmx', 3(1pe11.3))
      end
...
*  initial conditions.
  ...
  901 format('decid4')
  902 format('tolval, rtst',2(1pe11.3))
  903 format('thttol',1pe11.3)
      end
...
*  be doubled.
      ...
      return
  901 format('osc')
  902 format('im, ratdc, abdef, val',i5,3(1pe11.3))
  903 format('rmax, jsndif', 1pe11.3,i5)
  904 format('ave, avsm, avbg', 3(1pe11.3))
      end 
...
*  been passed.
      ...
      iflag = 0
      return
  901 format('fixed Jacobian iterations')
  902 format('Large residual, rnsq =',1pe12.4)
  903 format('iter, rnsq',i5,1pe11.3)
  904 format('failure of fixed Jacobian, iflag =',i5)
  905 format('fixed Jacobian convergence',i5,1pe11.3)
      end 
...
*  correspond to the final solution. 
  ...
  901 format('start Newton iterations')
  902 format(' iter',7x,'alfa',6x,'merit',7x,'rnsq')
  903 format('Too many Newton iterations')
  904 format('Watchdog tests fail, iter =', i5)
  905 format('Singular Jacobian, iter=',i5)
  906 format('Convergence, iter =',i5,4x,'rnsq =',1pe12.3)
  907 format('inform, alfa after getptq',i5,3x, 1pe11.3)
  908 format('alfa, merit, rnsq',3(1pe11.3))
  909 format(i5,3(1pe11.3))
  910 format('Newton iteration',i5)
      end 
...
*   99 format('iter,wmer,wbest,wprev',i5,3(1pe15.5))
*   98 format('itwtch,alfold',i5,1pe15.5)
      iflag = 0      
...
c      write(6,992) botblk(1,1),botblk(1,2)
  ...
  901 format('jaccal')
  902 format('topblk')
  903 format(i5,6(1pe11.3))
  904 format('ajac')
  905 format(2i5,5(1pe11.3))
  906 format('botblk')
      end 
...
*         write(6,904) (rhs(i), i=1,ncomp*nmsh)
  ...
  901 format('rhscal')
  902 format('rnsq',1pe11.3)
  903 format('rhs vector')
  904 format((7(1pe11.3)))
      end
...
*  that doubling the mesh produces too many points.)
  ...
  901 format('selmsh.  nmsh, ipow =',2i5)
  902 format('im, jcomp, ermeas, normalized er',2i5,2(1pe11.3))
  903 format('errmax',1pe11.3)
  904 format('nmest, irefin',(10i5))
  905 format('selmsh.  new nmsh =',i8)
  910 format('ihcomp',(10i5))
      end 
...
*  Refine the mesh.
  ...
  901 format('mshref. nummed, numbig =',2i5)
  902 format('ic, tst, bigst, second',i5, 3(1pe11.3))
  903 format('No significantly large value')
  904 format('numbig, nummed =',2i5)
  905 format('double the mesh')
      end 
...
*         if (pdebug)  write(6,901) im, it, errel, etest(it)
   ...
   10 continue
      ...
      return
  900 format('errest')
  901 format(2i5,2(1pe11.3))
  902 format('errsum',1pe11.3)
      end 
...
c
  990 continue 
c  990 if (debug) write (nout, 3000)
      return
c karline: toggled this off
c 1000 format(/ 31h alfmax  oldf    oldg    tolabs, 1p2e22.14, 1p2e16.8
c     *       / 31h alfuzz  epsaf   epsag   tolrel, 1p2e22.14, 1p2e16.8
c     *       / 31h crampd                        ,  l6)
c 1100 format(/ 31h alfa    ftry    ctry          , 1p2e22.14, 1pe16.8)
c 1200 format(/ 31h a       b       b - a   tol   , 1p2e22.14, 1p2e16.8
...
c 1300 format(  31h alfav   fv                    , 1p2e22.14 /)

c 2100 format(30h parabolic fit,    two points.)
c 2200 format(30h parabolic fit,  three points.)
c 2400 format(31h exponent reduced.  trial point, 1p1e22.14) 
c 2500 format(31h geo. bisection. xtry,daux,dtry, 1p3e22.14) 
c 2600 format(31h polynomial fit accepted.  xtry, 1p1e22.14) 
c 3000 format(53h ---------------------------------------------------- /)
c
c  end of getptq
      end 
