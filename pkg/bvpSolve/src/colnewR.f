...
C               if ISET(3) = 0 then colnew arbitrarily sets n = 5.
C
...
C
C     xconsts - is called once by  colnew  to initialize constants
...
cks: this cannot be printed - too long and unnecessary here...
c      WRITE (IOUT,260) NCOMP, (M(IP), IP=1,NCOMP)
c      GO TO 70
   60 CONTINUE
c   60 WRITE (IOUT,270) NCOMP, (M(IP), IP=1,NCOMP)
c   70 WRITE (IOUT,280) (ZETA(IP), IP=1,MSTAR)
c      IF ( NFXPNT .GT. 0 )
c     1   WRITE (IOUT,340) NFXPNT, (FIXPNT(IP), IP=1,NFXPNT)
c      WRITE (IOUT,290) K
c      WRITE (IOUT,300) (LTOL(IP), IP=1,NTOL)
c      WRITE (IOUT,310) (TOL(IP), IP=1,NTOL)
c      IF (IGUESS .GE. 2) WRITE (IOUT,320)
c      IF (IREAD .EQ. 2) WRITE (IOUT,330)
   80 CONTINUE
C
...
C
      ...
      DIMENSION DUMMY(1), SCALE(*), DSCALE(*), IPAR(*), RPAR(*)
...
C
   60      IF ( IPRINT .LT. 0 ) THEN
     ...
     2          3+IFREEZ, FSUB, DFSUB, GSUB, DGSUB, GUESS, RPAR, IPAR)
C
C...       check for a singular matrix
C
...
c           DO 410 J = 1, MSTAR
c             WRITE(IOUT,610) J
c  410      WRITE(IOUT,620) (Z(LJ), LJ = J, NZ, MSTAR)
C
...
C                   = 0  otherwise
C            iguess - ISET(9) in subroutine colnew.  it is used
...
C
  210        CALL VWBLOK (XCOL, HRHO, J, W(IW), V(IV), IPVTW(IDMZ),
     1       KD, ZVAL, DF, ACOL(1,J), DMZO(IDMZO), NCOMP, DFSUB, 
     2       MSING, RPAR, IPAR)
...
C
   ...
   60 GI(IROW,MSTAR+J) = DG(J)
     ...
     1             DF, ACOL, DMZO, NCOMP, DFSUB, MSING, RPAR, IPAR)
C
C**********************************************************************
C
C   purpose:
